...
from sound_mixer.overlay.mini_widget import BASE_APP_ICON_PX, BASE_FONT_PX, MUTED_OPACITY, MiniWidget
...
def test_mini_entry_centers_percentage_above_icon(mini):
    entry = mini._entries["aurora.exe"]
    layout = entry.layout()

    assert entry._volume_label.text() == "100%"
    assert layout.itemAt(0).widget() is entry._volume_label
    assert layout.itemAt(1).widget() is entry._icon_label
...
def test_mini_widget_applies_ui_scale(mini, settings):
