...
def test_mini_widget_appearance_and_whitelist_order_save_on_accept(qapp, settings):
    apps = []
    ...
    window = SettingsWindow(settings)
    assert not window._mini_widget_master_checkbox.isChecked()
    ...
    assert settings.get_mini_widget_show_master() is True
    assert settings.get_mini_widget_background_transparency() == 0.25
    assert settings.get_whitelist_apps() == list(reversed(apps))


def test_cancel_discards_mini_widget_appearance_changes(qapp, settings):
    window = SettingsWindow(settings)
    window._mini_widget_master_checkbox.setChecked(True)
    window._mini_widget_transparency_spinbox.setValue(0)
    window.reject()
